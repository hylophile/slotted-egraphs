...
// ???
// type ENodeOrVar<L> = PatternAst<L>;
type PENode<L> = (L, Vec<PatternAst<L>>);
...
impl<L: Language> Compiler<L> {
    ...
    // id is always root or children, so using our non-recexpr patternast should be fine
    fn add_todo(&mut self, pattern: &PatternAst<L>, reg: Reg) {
        match &pattern {
            PatternAst::PVar(v) => {
            ...
            PatternAst::ENode(n, _pat) => {
                self.todo_nodes.insert(reg, n.clone());
            }
            PatternAst::Subst(pattern_ast, pattern_ast1, pattern_ast2) => todo!(),
        }
    }

    fn load_pattern(&mut self, pattern: &PatternAst<L>) {
        self.free_vars = Vec::new();
        self.subtree_size = Vec::new();
